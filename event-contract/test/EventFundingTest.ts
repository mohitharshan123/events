...
describe("EventFunding", () => {
  ...
  it("should return if user is a donator correctly", async () => {
    await eventFunding
      .connect(donator1)
      .donateToEvent(eventId, { value: ethers.utils.parseEther("200") });
    const isDonator1ADonator = await eventFunding.isDonator(eventId, donator1.address);
    const isOwnerADonator = await eventFunding.isDonator(eventId, owner.address);
